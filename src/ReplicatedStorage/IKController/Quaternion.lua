...
function quaternion.FromToRotation(from,to)
	...
	local angle = math.acos(math.clamp(from.Unit:Dot(to.Unit), -1, 1));
	return quaternion.AngleAxis(angle, axis.Unit)
end

--aAngle in radian
function quaternion.AngleAxis(aAngle, aAxis)
	aAxis = aAxis.Unit;
	local rad = aAngle * 0.5;
	aAxis *= math.sin(rad);
	return quaternion.new(aAxis.X, aAxis.Y, aAxis.Z, math.sin(rad));
end
